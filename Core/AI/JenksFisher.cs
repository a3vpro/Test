...
namespace VisionNet.Core.IA
{
    ...
    public class JenksFisher
    {
        ...
        private void CalculateRange(int bi, int ei, int bp, int ep)
        {
            ...
            int mi = (int)System.Math.Floor((bi + ei) * 0.5);
            int mp = FindMaxBreakIndex(mi, bp, System.Math.Min(ep, mi + 1));

            // solve first half of the sub-problems with lower 'half' of possible outcomes
            CalculateRange(bi, mi, bp, System.Math.Min(mi, mp + 1));
