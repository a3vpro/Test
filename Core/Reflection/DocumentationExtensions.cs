    ...
    public static class DocumentationExtensions
    {
        ...
        private static XmlDocument XmlFromAssemblyNonCached(Assembly assembly)
        {
            ...
            if (!string.IsNullOrEmpty(assemblyFilename))
            {
                ...
                try
                {
                    var fullXmlFilePath = Path.ChangeExtension(assemblyFilename, ".xml");
                    if (!File.Exists(fullXmlFilePath))
                    {                        
                        var xmlFileName = Path.GetFileName(fullXmlFilePath);
                        fullXmlFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, xmlFileName);
                    }

                    streamReader = new StreamReader(fullXmlFilePath);
                }
